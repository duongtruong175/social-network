...
public class ImageSizeFilter extends Filter {
    ...
    public ImageSizeFilter(int mMinWidth, int mMinHeight, long mMaxSize) {
    ...
    public IncapableCause filter(Context context, Item item) {
        ...
        if (context != null && item != null) {
            if (bitmapBound.x < this.mMinWidth || bitmapBound.y < this.mMinHeight || item.size > this.mMaxSize) {
                String error = context.getString(R.string.error_size_image, this.mMinWidth, String.valueOf((int) PhotoMetadataUtils.getSizeInMB(this.mMaxSize)));
